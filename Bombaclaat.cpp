...
BomberBoard::BomberBoard() : currentPlayerIndex(0), currentAP(5) {
...
void BomberBoard::init() {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            if (grid[i][j] == nullptr && (rand() % 100 < 35)) {
                if ((i <= 1 && j <= 1) || (i >= 7 && j >= 7)) continue;
                grid[i][j] = new DestructibleWall(j, i);
            }
        }
    }
    players.push_back(new Player("Player 1", 0, 0));
    players.push_back(new Player("Player 2", 8, 8));
...
void BomberBoard::display() {
    ...
    for (int y = 0; y < height; ++y) {
        ...
        else if (y == 3) cout << "|      3,2,1 : Bomb (Timer)\n";
        else if (y == 4) cout << "|      F,S,B : Item Drop\n";
        else if (y == 5) cout << "|      X     : Explosion\n";
        else             cout << "|\n";
    }
    cout << " -------------------------------\n";
    cout << "=============================================================\n";
